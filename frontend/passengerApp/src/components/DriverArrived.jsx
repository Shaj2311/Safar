...
export const DriverArrived = ({ setCurrentScreen, onMenuClick, currentRideId }) => {
  ...
  useEffect(() => {
    ...
    const pollAndFetchDriver = async () => {
      ...
      try {
        // 1. Pehle ride ka status check karo
        const rideData = await getRideStatus(currentRideId);
        
        // 2. Agar ride complete ho gayi hai toh next screen pe bhejo
        if (isMounted && rideData.status === 'completed') {
          clearInterval(pollingInterval);
          setCurrentScreen('trip-completed');
          return;
        }

        // 3. Agar driver assign ho gaya hai aur profile abhi tak fetch nahi ki, toh fetch karo
    ...
    if (currentRideId) {
      pollAndFetchDriver(); // Immediate first check
